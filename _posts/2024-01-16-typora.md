...
Add Typora's repository (and package that includes add-apt-repository):
...
Install typora (and required library):
...
```
[3564:0121/111526.299903:ERROR:viz_main_impl.cc(183)] Exiting GPU process due to errors during initialization
```

Try launching `typora` with extra arguments `--disable-gpu`.

It should be pointed out that when installed in wattOS R13 Linux distribution (based on Debian 12 Bookworm) or ChromeOS, there is an application shortcut available which you can easily put in your application shelf/bar. This avoids the annoying warning messages from appearing in a terminal console. (`/usr/share/applications/typora.desktop`)
